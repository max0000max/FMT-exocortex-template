...
> Source-of-truth архитектуры: `DP.EXOCORTEX.001`. Навигация: `memory/navigation.md`.
...
> **Знай свои сервисы.** Перед началом работы определи, какие описания методов (сервисы) затронуты. Если не знаешь — СТОП → сверься с каталогом сервисов → предложи на согласование.
...
| Момент | Проверка |
|--------|---------|
| Перед началом работы над задачей | Какие сервисы затронуты? Нет ответа → СТОП |
...
| Межсистемное | Governance-репо `/PROCESSES.md` |
| Новая система | Сценарий + процессы + данные (ВДВ) |

> **Правило каталога:** Полный список сервисов → каталог сервисов. Детали реализации → `<system>/PROCESSES.md`.
> При добавлении/изменении сервиса: обновить ОБА места.
...
- **README.md:** Изменение структуры README.md — только по специальному согласованию с владельцем. Мелкие правки (опечатки, ссылки) допустимы без согласования.
