...
if $DRY_RUN; then
    ...
    echo "    max0000max → $GITHUB_USER"
    echo "    /home/user/IWE → $WORKSPACE_DIR"
    echo "    /home/user/.local/bin/kimi → $CLAUDE_PATH"
    echo "    -home-user-IWE → $CLAUDE_PROJECT_SLUG"
    echo "    4 → $TIMEZONE_HOUR"
    echo "    5:00 UTC → $TIMEZONE_DESC"
    echo "    /home/user → $HOME_DIR"
else
    find "$TEMPLATE_DIR" -type f \( -name "*.md" -o -name "*.json" -o -name "*.sh" -o -name "*.plist" -o -name "*.yaml" -o -name "*.yml" \) | while IFS= read -r file; do
        sed_inplace \
            -e "s|max0000max|$GITHUB_USER|g" \
            -e "s|/home/user/IWE|$WORKSPACE_DIR|g" \
            -e "s|/home/user/.local/bin/kimi|$CLAUDE_PATH|g" \
            -e "s|-home-user-IWE|$CLAUDE_PROJECT_SLUG|g" \
            -e "s|4|$TIMEZONE_HOUR|g" \
            -e "s|5:00 UTC|$TIMEZONE_DESC|g" \
            -e "s|/home/user|$HOME_DIR|g" \
            "$file"
    done
