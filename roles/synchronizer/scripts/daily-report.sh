...
LOG_DIR="/home/user/logs/synchronizer"
STRATEGY_DIR="/home/user/IWE/DS-strategy"

# Agent Workspace: если существует — отчёты идут туда
AGENT_WORKSPACE="/home/user/IWE/DS-agent-workspace"
...
STRATEGIST_LOG="/home/user/logs/strategist/$DATE.log"
...
"
        if echo "$warnings" | grep -q "push failed" 2>/dev/null; then
            report+="- **push failed:** Mac был оффлайн. Запусти \`cd /home/user/IWE/DS-strategy && git pull --rebase && git push\`
"
        fi
    else
        report+="
