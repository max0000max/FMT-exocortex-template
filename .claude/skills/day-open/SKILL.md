...
Коммиты за вчера по всем `/home/user/IWE/*/` репо. Сопоставить с DayPlan.
...
**Проверка обновлений:** `cd /home/user/IWE/FMT-exocortex-template && bash update.sh --check 2>&1`. Если доступно обновление → добавить в «Требует внимания»: «Доступно обновление IWE → `/iwe-update`».

**Проверка Base-репо (FPF, SPF, ZP):**
```bash
for repo in FPF SPF ZP; do
  dir="/home/user/IWE/$repo"
